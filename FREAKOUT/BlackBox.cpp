...
#include <ddraw.h>			// diterctX includes
...
LPDIRECTDRAW7			lpdd			= NULL;		// dd bject
...
DDSURFACEDESC2			ddsd;						// a direct draw surface desription struct
DDBLTFX					ddbltfx;					// used to fill
DDSCAPS2				ddscaps;					// a direct draw surface cpablities struct
HRESULT					ddrval;						// result back from ddcalls
...
// this function initalizes directdraw
int DD_Init(int width, int height, int bpp)
{
	...
	// set the cooperation level to the windowed mode normal
	...
	// flippable surface structure, set the flags for that
	ddsd.ddsCaps.dwCaps = 
	...
	// query for the back buffer i.e the secondary surface
	...
	// create palette data
	// clear all entries definsive programming
	...
	for (index = 0; index < 256; index++)
	{
		...
		else if (index >= 64 && index < 128)   // shades of green
			palette[index].peGreen = (index-64)*4;
		else if (index >= 128 && index < 192)	// shades of blue
			palette[index].peBlue = (index-128)*4;
		else if (index >= 192 && index < 255)	// shades of gray
...
LPDIRECTDRAWCLIPPER DD_Attach_Clipper(LPDIRECTDRAWSURFACE7 lpdds,
									  int num_rects,
									  LPRECT clip_list)
// this function creates a clipper from the sent clip list and ataches
...
// count, use in conjugation with Wait_Clock()
{
...
// this function is used to wait for specific number of clicks
...
int DD_Fill_Surface(LPDIRECTDRAWSURFACE7 lpdds, int color)
{
	...
	// clear out the structure and set the size feild
	DD_INIT_STRUCT(ddbltfx);

	// set the dwfillcolor feild to the desired color
	ddbltfx.dwFillColor = color;

	// ready to blt surface
	lpdds->Blt(NULL,											// pointer to dest rectangle
			   NULL,											// pointer to source surface, NA
...
{
	DDBLTFX ddbltfx;		// this contains DDBLTFX structure
	RECT fill_area;			// this contains the destination rectangle

	// clearout the structure and set the size feild
	DD_INIT_STRUCT(ddbltfx);
	...
	// ready to blt surface, in this case blt to primary
	lpdds->Blt(&fill_area,		// ptr to dest recatangle
...
// this function draws the sent text on the surface
